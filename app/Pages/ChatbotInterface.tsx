...
import { faPaperPlane, faRobot, faTimes, faCommentDots } from '@fortawesome/free-solid-svg-icons';
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      role: 'assistant',
      content: 'Hello! I can help you swap tokens or answer crypto questions. Try asking me to swap tokens or about crypto prices.',
      timestamp: new Date()
    }
  ]);
    ...
    try {
      ...
      if (!apiKey || apiKey.trim() === '') {
        const fallbackMessage: ChatMessage = {
          id: Date.now().toString(),
          role: 'assistant',
          content: "I'm sorry, I can't connect to my knowledge base right now. You can still use these commands:\n\n" +
            "• For swaps: 'swap [amount] [token] for [token]'. Example: 'swap 5 USDC for ETH'\n" +
      ...
      const systemPrompt = `You are a helpful assistant for a cryptocurrency application on the BASE blockchain. 
      You can help users swap tokens and answer questions about cryptocurrency.
      
      Available tokens: ${availableTokens.map(t => t.symbol).join(', ')}
      
      If the user wants to swap tokens, extract the following information:
      1. Source token (from)
      2. Destination token (to)
      3. Amount to swap
      
      Format swap requests as: SWAP_REQUEST:{fromToken}:{toToken}:{amount}
      
      For price queries, respond with the latest information you have.
      Keep responses concise and focused on crypto topics.`;
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-3.5-turbo', // Changed from gpt-4-turbo to gpt-3.5-turbo which is more widely available
          messages: [
      ...
      // Check if the response contains a swap request
      if (aiResponse.includes('SWAP_REQUEST:')) {
        ...
        if (swapMatch) {
          ...
          const fromToken = availableTokens.find(t => 
            t.symbol.toLowerCase() === fromTokenSymbol.toLowerCase().trim());
          const toToken = availableTokens.find(t => 
            t.symbol.toLowerCase() === toTokenSymbol.toLowerCase().trim());
          
          if (fromToken && toToken) {
            // Remove the SWAP_REQUEST format from the displayed message
            aiResponse = aiResponse.replace(/SWAP_REQUEST:[^:]+:[^:]+:[^:]+/, 
              `I'll help you swap ${amount} ${fromToken.symbol} to ${toToken.symbol}.`);
            ...
            try {
              ...
              // Add a system message about the swap result
              const resultMessage: ChatMessage = {
                id: Date.now().toString() + '-result',
                role: 'system',
                content: success 
      ...
      if (error instanceof Error) {
        if (error.message.includes("404")) {
          ...
          errorContent = "There seems to be an authentication issue with my AI service. In the meantime, you can still use direct commands for swaps and price checks.";
        } else if (error.message.includes("429")) {
          errorContent = "I've reached my usage limit for the moment. Please try again in a few minutes, or use direct commands for swaps and price checks.";
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 bg-white text-black p-4 rounded-full shadow-lg hover:bg-[#d3c81a] transition-colors z-50"
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-6 w-80 sm:w-96 h-[500px] bg-gray-900 border border-gray-700 rounded-lg shadow-xl flex flex-col z-40">
          ...
          <div className="p-4 border-b border-gray-700 bg-gray-800 rounded-t-lg flex items-center">
            <FontAwesomeIcon icon={faRobot} className="text-[#d3c81a] mr-2" />
            <h3 className="text-white font-bold">Crypto Assistant</h3>
            <button 
          ...
          <div className="p-4 border-t border-gray-700 bg-gray-800 rounded-b-lg">
            ...
            <div className="flex items-center">
              <textarea
                ...
                placeholder="Ask about crypto or request a swap..."
